            ...
            <div class="section__panel block-form flex flex-col justify-center items-center">
                <form class="block-form__form form w-full flex flex-col">
                    ...
                    <div class="form__row row flex flex-col">
                        <a-input class="w-full row__item" v-model:value="this.userData.username" name="username" placeholder="имя" />
                        <div class="w-full row__item">
                            <a-input v-model:value="this.userData.email" name="email" placeholder="email" />
                            <div class="input-message-box flex items-center mt-1">
                                <template v-if="validEmail">
                                    <i class='bx bx-check-circle success'></i> <span class="success">Корректный формат email</span>
                                </template>
                                <template v-if="!validEmail && this.userData.email !== ''">
                                    <i class='bx bx-error-circle error'></i> <span class="error">Некорректный формат email</span>
                                </template>
                            </div>
                        </div>
                        <div class="w-full row__item">
                            <a-input-password v-model:value="this.userData.password" type="password" name="password" placeholder="пароль" />
                            <div class="wrapp-progress px-2 mt-1">
                            ...
                            <div class="input-message-box flex items-center mt-1">
                                <template v-if="getProgress === 0">
                                </template>
                                <template v-else-if="getProgress <= 20">
                                    <span class="error">Слабый пароль</span>
                                </template>
                                <template v-else-if="getProgress <= 60">
                                    <span class="danger">Пароль средней сложности</span>
                                </template>
                                <template v-else-if="getProgress <= 80">
                                    <span class="warning">Пароль высокой сложности</span>
                                </template>
                                <template v-else-if="getProgress >= 100">
                                    <span class="success">Надежность пароля максимальна</span>
                                </template>
                            </div>
                        </div>
                    </div>
                    <div class="form__row flex items-center">
                        <a-button type="primary"
                                  class="w-fit" :size="sizeButton">
                            Зарегистрироваться
                        </a-button>
                        <router-link :to="{name: 'auth'}" class="link ml-3">
                            войти в систему
                        </router-link>
                    </div>
                </form>
